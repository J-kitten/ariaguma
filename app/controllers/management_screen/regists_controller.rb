...
class ManagementScreen::RegistsController < ManagementScreen::BaseController
  ...
  def send_first_book  # regists 予約者へ返信
    ...
    begin
      RegistMailer.download_email(@regist).deliver_now
    rescue => e
    ...
    RegistsReply.create!(
      name: @regist.name,
      subject: "『幻魔との死闘』ダウンロード先URLのメール送信完了",
      email_hash: @regist.email_hash,
      message: "2冊目の電子書籍のダウンロード先URLをメールで送信しました。（#{Time.current.strftime('%Y/%m/%d %H:%M:%S')}）",
      regist_id: @regist.id
    )
  ...
  def send_second_email
    ...
    RegistsReply.create!(
      name: @regist.name,
      subject: '2冊目のダウンロード先URLのメール送信完了',
      email_hash: @regist.email_hash,
      message: "2冊目の電子書籍のダウンロード先URLをメールで送信しました。",
      regist_id: @regist.id
    )
